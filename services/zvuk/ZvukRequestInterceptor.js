 ...
 * Request interceptor for Zvuk service: auth token capture, keyserver headers capture, early injection
 * @module services/zvuk/ZvukRequestInterceptor
 * @author ivanvit
 * @version 0.0.1
 */
...
(function() {
    ...
    globalThis.serviceRequestInterceptors.push({
        serviceName: 'zvuk',
        authUrls: ['https://zvuk.com/*', 'https://*.zvuk.com/*'],
        keyUrls: ['*://zvuk.com/keyserver/api/v1/key*'],

        setupKeyCapture(browserAPI, _isFirefox) {
            if (!browserAPI?.webRequest) return;

            const KEY_URLS = this.keyUrls;
                    ...
                    try {
                        ...
                        filter.onstop = () => {
                            try {
                                ...
                                console.log('[KeyCapture] native response for track',
                                    trackId, ':', globalThis.nativeKeyStore[trackId]);
                            } catch (e) {
                        ...
                        filter.onerror = () =>
                            console.warn('[KeyCapture] filter error for track', trackId, filter.error);
                    } catch (e) {
                        ...
                        func: () => {
                            ...
                            window.fetch = async function(...args) {
                                ...
                                try {
                                    ...
                                    const isCdnApi = /zvuk\.com\/(?:api|gateway|track)/.test(url);
                                    if (isCdnApi && !url.includes('cdn-hls-slicer')) {
                                                ...
                                                for (const streamUrl of found) {
                                                    ...
                                                    if (m)
                                                        {window.postMessage({
                                                            ...
                                                            cdnTrackId: m[1], streamUrl, apiUrl: url
                                                        }, '*');}
                                                }
                                            } catch {}
                                        }).catch(() => {});
    ...
    console.log('[ZvukRequestInterceptor] Loaded');
})();
